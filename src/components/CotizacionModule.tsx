  ...
  return (
    <div className="space-y-5 pb-16 animate-in fade-in duration-200">
      ...
      {isPrintModalOpen && printRecord && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 print:p-0 print:bg-transparent">
          <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden border border-slate-200 flex flex-col max-h-[92vh] print:max-h-none print:shadow-none print:border-none print:rounded-none">
            {/* Cabecera del Modal (Oculta en Impresión) */}
            <div className="p-4 bg-slate-900 text-white flex items-center justify-between print:hidden">
              <div className="flex items-center gap-2">
                <Printer className="w-4 h-4 text-slate-300" />
                <span className="text-xs font-bold tracking-wide uppercase">
                  Vista Previa de Impresión — Cotización #{printRecord.numeroCotizacion}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => downloadPdf(printRecord)}
                ...
                <button
                  ...
                  className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer shadow-xs"
                >
                  <Printer className="w-3.5 h-3.5" />
            ...
            {/* CONTENIDO IMPRIMIBLE FORMAL Y MINIMALISTA */}
            <div className="p-6 sm:p-8 overflow-y-auto print:p-4 print:overflow-visible space-y-6 text-slate-900 bg-white">
              ...
              <div className="flex items-start justify-between border-b-2 border-slate-900 pb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-600 font-medium mt-0.5">{company.address}</p>
                  <p className="text-xs text-slate-500">
                    Tel: {company.phone} • Email: {company.email}
                  </p>
                  <p className="text-2xs text-slate-400 uppercase tracking-wider font-semibold mt-0.5">
                    {company.authorizedCenter}
                  </p>
                </div>

                <div className="text-right border-2 border-slate-900 p-2.5 rounded-lg bg-slate-50 min-w-[170px]">
                  ...
                  <span className="text-xl font-black text-rose-600 font-mono block">
                    #{printRecord.numeroCotizacion}
                  </span>
                  {printRecord.referenciaRef && (
                    <span className="text-2xs font-semibold text-slate-600 font-mono block mt-0.5">
                      REF: {printRecord.referenciaRef}
                    </span>
                  )}
                  <span className="inline-block mt-1 px-2 py-0.5 rounded text-2xs font-bold bg-slate-900 text-white uppercase">
                    {printRecord.estatus}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs border border-slate-300 rounded-lg p-3 bg-slate-50/50">
                <div>
                  <span className="text-2xs font-bold text-slate-400 uppercase block">
                    Fecha de Pedido
                  </span>
                  <span className="font-semibold text-slate-800">
                    {printRecord.fechaPedido || 'N/A'}
                  </span>
                </div>
                <div>
                  <span className="text-2xs font-bold text-slate-400 uppercase block">Atendió</span>
                  <span className="font-semibold text-slate-800">
                    {printRecord.atendio || 'N/A'}
                  </span>
                </div>
                <div>
                  <span className="text-2xs font-bold text-slate-400 uppercase block">
                  ...
                  <span className="font-semibold text-slate-800 font-mono">
                    {printRecord.celular || printRecord.telefono || 'N/A'}
                  </span>
                </div>
                <div>
                  <span className="text-2xs font-bold text-slate-400 uppercase block">Email @</span>
                  <span className="font-semibold text-slate-800 truncate block">
                    {printRecord.email || 'N/A'}
                  </span>
                </div>
                ...
                <div className="col-span-2 pt-2 border-t border-slate-200">
                  ...
                  <span className="font-semibold text-slate-800">
                    {printRecord.aparato} — {printRecord.marca} | Mod: {printRecord.modelo}{' '}
                    {printRecord.serie ? `(Serie: ${printRecord.serie})` : ''}
                  </span>
                </div>
              </div>

              {/* Detalle de la Refacción Solicitada */}
              <div className="border border-slate-300 rounded-lg overflow-hidden">
                <div className="bg-slate-900 text-white px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider flex justify-between">
                  <span>Nombre o Número de Parte Solicitada</span>
                  <span>Importe</span>
                </div>
                <div className="p-3.5 bg-white space-y-2">
                  <p className="text-xs font-medium text-slate-800 whitespace-pre-wrap leading-relaxed">
                    {printRecord.nombreNumeroParte || 'Sin descripción de pieza detallada'}
                  </p>
                </div>
              </div>

              {/* Política de Garantía Fija */}
              <div className="p-3 bg-rose-50/60 border border-rose-200 rounded-lg text-xs text-rose-900">
                <span className="font-bold uppercase tracking-wide">AVISO DE POLÍTICA Y GARANTÍA: </span>
                <span>{printRecord.notaPolitica || FIXED_POLICY_NOTE}</span>
              </div>

              {/* Detalles de la Operación y Logística */}
              {(printRecord.detallesOperacion || printRecord.datosPedido) && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  {printRecord.detallesOperacion && (
              ...
              <div className="flex justify-end">
                <div className="w-full max-w-xs border border-slate-300 rounded-lg overflow-hidden">
                  <div className="bg-slate-100 px-3.5 py-1.5 text-2xs font-bold text-slate-600 uppercase border-b border-slate-200">
                    Resumen de Importes
                  </div>
              ...
              <div className="pt-8 grid grid-cols-2 gap-8 text-center text-xs">
                ...
                <div className="space-y-1">
                  <div className="border-b border-slate-400 h-8"></div>
                  <span className="font-bold text-slate-800 block text-2xs uppercase">
                    Cotizó / Asesor Técnico
                  </span>
                  <span className="text-2xs text-slate-500 block truncate">
                    {printRecord.atendio || 'Personal de Atención'}
                  </span>
                </div>
              </div>

              {/* Pie de Página Formal */}
              <div className="pt-4 border-t border-slate-200 text-center text-2xs text-slate-400">
                <span>
                  Comprobante oficial de cotización y presupuesto de partes. Documento para control interno y seguimiento.
                </span>
              </div>
            </div>
          </div>
