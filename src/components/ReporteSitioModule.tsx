import React, { useState, useMemo } from 'react';
import {
  Plus,
  Search,
  FileDown,
  FileSpreadsheet,
  Trash2,
  Edit,
  Eye,
  X,
  Save,
  MapPin,
  Clock,
  Lock,
} from 'lucide-react';
import { ReporteSitio, CompanyInfo } from '../types';
import { StorageService } from '../lib/storage';
import { ExportService } from '../lib/exportUtils';
import { STAFF_LIST, APPLIANCE_CATEGORIES, BRANDS_LIST } from '../data/initialData';
import { FormatPrintPreview } from './FormatPrintPreview';

const TIPO_SERVICIO_OPTIONS: ReporteSitio['tipoServicio'][] = [
  'Con cargo',
  'Garantía',
  'Mantenimiento',
  'Revisión técnica',
  'Instalación',
];

const TIPO_CASA_OPTIONS: ReporteSitio['tipoCasa'][] = [
  'Casa de 1 piso',
  'Casa de 2 pisos',
  'Departamento',
  'Local comercial',
  'Oficina',
  'Privada/Condominio',
];

const HORA_VISITA_OPTIONS = [
  '09:00 AM - 11:00 AM',
  '11:00 AM - 01:00 PM',
  '01:00 PM - 03:00 PM',
  '03:00 PM - 05:00 PM',
  '05:00 PM - 07:00 PM',
  'Horario abierto / Por confirmar',
];

const formatCurrency = (val: number | undefined): string => {
  if (val === undefined || isNaN(val)) return '$0.00';
  return new Intl.NumberFormat('es-MX', {
    style: 'currency',
    currency: 'MXN',
  }).format(val);
};

export const ReporteSitioModule: React.FC<{ company: CompanyInfo }> = ({ company }) => {
  const [reportes, setReportes] = useState<ReporteSitio[]>(() => StorageService.getReportesSitio());
  const [searchQuery, setSearchQuery] = useState('');
  const [tipoFilter, setTipoFilter] = useState<string>('all');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<ReporteSitio | null>(null);
  const [previewRecord, setPreviewRecord] = useState<ReporteSitio | null>(null);

  const initialFormState: Partial<ReporteSitio> = {
    numeroReporte: StorageService.getNextReporteNumber(),
    tipoServicio: 'Con cargo',
    fechaReporte: new Date().toISOString().split('T')[0],
    atendio: STAFF_LIST[0],
    nombreCliente: '',
    direccion: '',
    colonia: '',
    tipoCasa: 'Casa de 1 piso',
    telefono: '',
    celular: '',
    aparato: APPLIANCE_CATEGORIES[0],
    marca: BRANDS_LIST[0],
    modelo: '',
    serieDifusor: '',
    serieEquipo: '',
    fallaReportada: '',
    fechaVisita: new Date().toISOString().split('T')[0],
    horaVisita: HORA_VISITA_OPTIONS[1],
    tecnico: STAFF_LIST[6] || STAFF_LIST[1],
    detalles1erVisita: '',
    detalles2daVisita: '',
    detalles3eraVisita: '',
    presupuesto: 0,
    partesSolicitadas: '',
    numeroPedido: '',
    observaciones: '',
    numeroOrdenServicio: '',
    informacionConfidencial: '',
  };

  const [formData, setFormData] = useState<Partial<ReporteSitio>>(initialFormState);

  const openNewModal = () => {
    setEditingItem(null);
    setFormData({
      ...initialFormState,
      numeroReporte: StorageService.getNextReporteNumber(),
      fechaReporte: new Date().toISOString().split('T')[0],
      fechaVisita: new Date().toISOString().split('T')[0],
    });
    setIsModalOpen(true);
  };

  const openEditModal = (item: ReporteSitio) => {
    setEditingItem(item);
    setFormData({ ...item });
    setIsModalOpen(true);
  };

  const handleDelete = (id: string) => {
    if (window.confirm('¿Está seguro de eliminar este reporte de servicio en sitio?')) {
      const updated = StorageService.deleteReporteSitio(id);
      setReportes(updated);
    }
  };

  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.nombreCliente?.trim()) {
      alert('Por favor ingrese el nombre del cliente');
      return;
    }

    const itemToSave: ReporteSitio = {
      id: editingItem ? editingItem.id : `rep-${Date.now()}`,
      createdAt: editingItem ? editingItem.createdAt : new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      numeroReporte: formData.numeroReporte || StorageService.getNextReporteNumber(),
      tipoServicio: formData.tipoServicio || 'Con cargo',
      fechaReporte: formData.fechaReporte || new Date().toISOString().split('T')[0],
      atendio: formData.atendio || STAFF_LIST[0],
      nombreCliente: formData.nombreCliente || '',
      direccion: formData.direccion || '',
      colonia: formData.colonia || '',
      tipoCasa: formData.tipoCasa || 'Casa de 1 piso',
      telefono: formData.telefono || '',
      celular: formData.celular || '',
      aparato: formData.aparato || APPLIANCE_CATEGORIES[0],
      marca: formData.marca || BRANDS_LIST[0],
      modelo: formData.modelo || '',
      serieDifusor: formData.serieDifusor || '',
      serieEquipo: formData.serieEquipo || '',
      fallaReportada: formData.fallaReportada || '',
      fechaVisita: formData.fechaVisita || new Date().toISOString().split('T')[0],
      horaVisita: formData.horaVisita || HORA_VISITA_OPTIONS[1],
      tecnico: formData.tecnico || STAFF_LIST[6] || STAFF_LIST[1],
      detalles1erVisita: formData.detalles1erVisita || '',
      detalles2daVisita: formData.detalles2daVisita || '',
      detalles3eraVisita: formData.detalles3eraVisita || '',
      presupuesto: Number(formData.presupuesto || 0),
      partesSolicitadas: formData.partesSolicitadas || '',
      numeroPedido: formData.numeroPedido || '',
      observaciones: formData.observaciones || '',
      numeroOrdenServicio: formData.numeroOrdenServicio || '',
      informacionConfidencial: formData.informacionConfidencial || '',
    };

    const updated = StorageService.saveReporteSitio(itemToSave);
    setReportes(updated);
    setIsModalOpen(false);
  };

  const filtered = useMemo(() => {
    return reportes.filter((item) => {
      const matchesSearch =
        item.numeroReporte.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.nombreCliente.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.direccion.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.colonia.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.modelo.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.tecnico.toLowerCase().includes(searchQuery.toLowerCase());

      const matchesTipo = tipoFilter === 'all' || item.tipoServicio === tipoFilter;
      return matchesSearch && matchesTipo;
    });
  }, [reportes, searchQuery, tipoFilter]);

  const totalPresupuesto = useMemo(() => {
    return reportes.reduce((acc, curr) => acc + (curr.presupuesto || 0), 0);
  }, [reportes]);

  const garantiaCount = useMemo(() => {
    return reportes.filter((item) => item.tipoServicio === 'Garantía').length;
  }, [reportes]);

  return (
    <div className="space-y-4">
      {/* Bento Grid Metrics Summary Bar */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:border-slate-300 transition-all">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
            Total Reportes en Sitio
          </p>
          <div className="flex items-baseline justify-between">
            <h3 className="text-xl font-bold text-slate-800">{reportes.length}</h3>
            <span className="text-[11px] font-semibold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100">
              Registrados
            </span>
          </div>
        </div>

        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:border-slate-300 transition-all">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
            En Garantía
          </p>
          <div className="flex items-baseline justify-between">
            <h3 className="text-xl font-bold text-emerald-600">{garantiaCount}</h3>
            <span className="text-[11px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">
              Sin cargo
            </span>
          </div>
        </div>

        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:border-slate-300 transition-all">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
            Total Presupuestado
          </p>
          <div className="flex items-baseline justify-between">
            <h3 className="text-xl font-bold text-indigo-600">{formatCurrency(totalPresupuesto)}</h3>
            <span className="text-[11px] font-semibold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100">
              Acumulado
            </span>
          </div>
        </div>

        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:border-slate-300 transition-all">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
            Modalidad
          </p>
          <div className="flex items-baseline justify-between">
            <h3 className="text-xl font-bold text-amber-600">En Sitio</h3>
            <span className="text-[11px] font-semibold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100">
              A Domicilio
            </span>
          </div>
        </div>
      </div>

      {/* Header & Controls Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
        <div className="flex items-center gap-3">
          <h2 className="font-bold text-base text-slate-800 flex items-center gap-2">
            <span>Servicio en Sitio (A Domicilio)</span>
            <span className="bg-indigo-100 text-indigo-700 text-[10px] px-2 py-0.5 rounded font-bold uppercase">
              Módulo 4
            </span>
          </h2>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <button
            onClick={() => {
              if (reportes.length > 0) {
                ExportService.exportToPdf('reporte_sitio', reportes[0]);
              }
            }}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors shadow-2xs"
          >
            <span className="text-rose-500 font-black text-[10px]">PDF</span>
            <span>Exportar</span>
          </button>
          <button
            onClick={() => ExportService.exportModuleListToExcel('reporte_sitio')}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors shadow-2xs"
          >
            <span className="text-emerald-600 font-black text-[10px]">XLS</span>
            <span>Reporte</span>
          </button>
          <button
            onClick={openNewModal}
            className="flex items-center gap-1.5 px-4 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 transition-colors shadow-xs"
          >
            <Plus className="w-3.5 h-3.5" />
            <span>Nuevo Reporte en Sitio</span>
          </button>
        </div>
      </div>

      {/* Filter and Search Bar */}
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-white p-3 rounded-xl border border-slate-200 shadow-xs">
        <div className="relative w-full sm:w-80">
          <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
          <input
            type="text"
            placeholder="Buscar por # de reporte, cliente, dirección, técnico..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white text-slate-800"
          />
        </div>

        <div className="flex items-center gap-2 w-full sm:w-auto">
          <span className="text-xs text-slate-500 font-medium whitespace-nowrap">Tipo de Servicio:</span>
          <select
            value={tipoFilter}
            onChange={(e) => setTipoFilter(e.target.value)}
            className="text-xs bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
            <option value="all">Todos los tipos</option>
            {TIPO_SERVICIO_OPTIONS.map((st) => (
              <option key={st} value={st}>
                {st}
              </option>
            ))}
          </select>
        </div>
      </div>

      {/* Table Data (Bento Card Container) */}
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200">
              <tr>
                <th className="p-3.5"># Reporte / Fecha</th>
                <th className="p-3.5">Cliente / Domicilio</th>
                <th className="p-3.5">Tipo Servicio</th>
                <th className="p-3.5">Aparato / Marca / Modelo</th>
                <th className="p-3.5">Técnico / Cita Visita</th>
                <th className="p-3.5 text-right">Presupuesto</th>
                <th className="p-3.5 text-center">Acciones</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {filtered.length > 0 ? (
                filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-indigo-50/30 transition-colors">
                    <td className="p-3.5">
                      <span className="font-bold text-indigo-700 font-mono block">{item.numeroReporte}</span>
                      <span className="text-[11px] text-slate-400">{item.fechaReporte}</span>
                    </td>
                    <td className="p-3.5">
                      <span className="font-semibold text-slate-800 block">{item.nombreCliente}</span>
                      <span className="text-[11px] text-slate-500 truncate block max-w-xs">{item.direccion}</span>
                    </td>
                    <td className="p-3.5">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium ${
                        item.tipoServicio === 'Garantía'
                          ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                          : 'bg-indigo-50 text-indigo-700 border border-indigo-200'
                      }`}>
                        {item.tipoServicio}
                      </span>
                    </td>
                    <td className="p-3.5">
                      <span className="text-slate-800 font-medium block">{item.aparato} • {item.marca}</span>
                      <span className="text-[11px] text-slate-500">Mod: {item.modelo}</span>
                    </td>
                    <td className="p-3.5">
                      <span className="text-slate-800 font-medium block">{item.tecnico}</span>
                      <span className="text-[11px] text-slate-500">{item.fechaVisita} {item.horaVisita}</span>
                    </td>
                    <td className="p-3.5 text-right font-bold text-slate-800">
                      {formatCurrency(item.presupuesto)}
                    </td>
                    <td className="p-3.5 text-center">
                      <div className="inline-flex items-center gap-1">
                        <button
                          onClick={() => setPreviewRecord(item)}
                          className="p-1.5 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
                          title="Ver Formato / Imprimir"
                        >
                          <Eye className="w-3.5 h-3.5" />
                        </button>
                        <button
                          onClick={() => ExportService.exportToPdf('reporte_sitio', item)}
                          className="p-1.5 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded-lg transition-colors"
                          title="Descargar PDF"
                        >
                          <FileDown className="w-3.5 h-3.5" />
                        </button>
                        <button
                          onClick={() => ExportService.exportRecordToExcel('reporte_sitio', item)}
                          className="p-1.5 text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 rounded-lg transition-colors"
                          title="Descargar Excel"
                        >
                          <FileSpreadsheet className="w-3.5 h-3.5" />
                        </button>
                        <button
                          onClick={() => openEditModal(item)}
                          className="p-1.5 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
                          title="Editar"
                        >
                          <Edit className="w-3.5 h-3.5" />
                        </button>
                        <button
                          onClick={() => handleDelete(item.id)}
                          className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"
                          title="Eliminar"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    </td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-slate-400">
                    No se encontraron reportes en sitio con los criterios seleccionados.
                  </td>
                </tr>
              )}
            </tbody>
          </table>
        </div>
      </div>

      {/* CREATE / EDIT MODAL (Bento Grid Architecture) */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4">
          <div className="bg-[#F1F5F9] w-full max-w-5xl rounded-2xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col border border-slate-200">
            {/* Modal Header */}
            <div className="px-6 py-3.5 bg-white border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 bg-indigo-600 rounded-lg flex items-center justify-center text-white text-xs font-bold">
                  RS
                </div>
                <div>
                  <h3 className="text-sm sm:text-base font-bold text-slate-800">
                    {editingItem ? 'Editar Reporte en Sitio' : 'Nuevo Reporte de Servicio a Domicilio'}
                  </h3>
                  <p className="text-[11px] text-slate-400">
                    {formData.numeroReporte} • Visitas técnicas, diagnóstico en domicilio y costeo
                  </p>
                </div>
              </div>
              <button
                onClick={() => setIsModalOpen(false)}
                className="p-1.5 text-slate-400 hover:text-slate-700 rounded-lg hover:bg-slate-100 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            {/* Bento Grid 12-column Form Layout */}
            <form onSubmit={handleSave} className="p-4 sm:p-6 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                {/* Column 1: Left (4 cols) */}
                <div className="md:col-span-4 space-y-4">
                  {/* Bento Card: Encabezado y Control */}
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
                    <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-3">
                      Encabezado y Control
                    </h3>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1 col-span-2">
                        <label className="text-[10px] font-semibold text-slate-500"># de Reporte</label>
                        <input
                          type="text"
                          readOnly
                          value={formData.numeroReporte}
                          className="text-xs bg-slate-100 border border-slate-200 rounded-lg px-2.5 py-1.5 font-mono font-bold text-indigo-600 cursor-not-allowed"
                        />
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Tipo Servicio</label>
                        <select
                          value={formData.tipoServicio}
                          onChange={(e) => setFormData({ ...formData, tipoServicio: e.target.value as any })}
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        >
                          {TIPO_SERVICIO_OPTIONS.map((st) => (
                            <option key={st} value={st}>
                              {st}
                            </option>
                          ))}
                        </select>
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Fecha Reporte</label>
                        <input
                          type="date"
                          value={formData.fechaReporte}
                          onChange={(e) => setFormData({ ...formData, fechaReporte: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        />
                      </div>
                      <div className="flex flex-col gap-1 col-span-2">
                        <label className="text-[10px] font-semibold text-slate-500">Atendió</label>
                        <select
                          value={formData.atendio}
                          onChange={(e) => setFormData({ ...formData, atendio: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white truncate"
                        >
                          {STAFF_LIST.map((staff) => (
                            <option key={staff} value={staff}>
                              {staff}
                            </option>
                          ))}
                        </select>
                      </div>
                    </div>
                  </div>

                  {/* Bento Card: Cliente y Domicilio */}
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
                    <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-3">
                      Cliente y Domicilio
                    </h3>
                    <div className="space-y-3">
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">
                          Nombre del Cliente <span className="text-rose-500">*</span>
                        </label>
                        <input
                          type="text"
                          required
                          placeholder="Nombre del cliente..."
                          value={formData.nombreCliente}
                          onChange={(e) => setFormData({ ...formData, nombreCliente: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white font-medium"
                        />
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Tipo de Inmueble</label>
                        <select
                          value={formData.tipoCasa}
                          onChange={(e) => setFormData({ ...formData, tipoCasa: e.target.value as any })}
                          className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        >
                          {TIPO_CASA_OPTIONS.map((tc) => (
                            <option key={tc} value={tc}>
                              {tc}
                            </option>
                          ))}
                        </select>
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Dirección y Referencias</label>
                        <input
                          type="text"
                          placeholder="Calle, No., entre calles..."
                          value={formData.direccion}
                          onChange={(e) => setFormData({ ...formData, direccion: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        />
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Colonia / Sector</label>
                        <input
                          type="text"
                          placeholder="Colonia o fraccionamiento..."
                          value={formData.colonia}
                          onChange={(e) => setFormData({ ...formData, colonia: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        />
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="flex flex-col gap-1">
                          <label className="text-[10px] font-semibold text-slate-500">Teléfono</label>
                          <input
                            type="tel"
                            placeholder="5559871122"
                            value={formData.telefono}
                            onChange={(e) => setFormData({ ...formData, telefono: e.target.value })}
                            className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                          />
                        </div>
                        <div className="flex flex-col gap-1">
                          <label className="text-[10px] font-semibold text-slate-500">Celular</label>
                          <input
                            type="tel"
                            placeholder="5598765432"
                            value={formData.celular}
                            onChange={(e) => setFormData({ ...formData, celular: e.target.value })}
                            className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                          />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                {/* Column 2: Middle (5 cols) */}
                <div className="md:col-span-5 space-y-4">
                  {/* Bento Card: Identificación del Equipo */}
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
                    <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-3">
                      Identificación del Equipo
                    </h3>
                    <div className="grid grid-cols-3 gap-2 mb-3">
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Aparato</label>
                        <select
                          value={formData.aparato}
                          onChange={(e) => setFormData({ ...formData, aparato: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        >
                          {APPLIANCE_CATEGORIES.map((cat) => (
                            <option key={cat} value={cat}>
                              {cat}
                            </option>
                          ))}
                        </select>
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Marca</label>
                        <select
                          value={formData.marca}
                          onChange={(e) => setFormData({ ...formData, marca: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        >
                          {BRANDS_LIST.map((b) => (
                            <option key={b} value={b}>
                              {b}
                            </option>
                          ))}
                        </select>
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Modelo</label>
                        <input
                          type="text"
                          placeholder="Ej. WET4027"
                          value={formData.modelo}
                          onChange={(e) => setFormData({ ...formData, modelo: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Serie Difusor / Int.</label>
                        <input
                          type="text"
                          placeholder="Serie difusor..."
                          value={formData.serieDifusor}
                          onChange={(e) => setFormData({ ...formData, serieDifusor: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        />
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Serie Equipo / Ext.</label>
                        <input
                          type="text"
                          placeholder="Serie equipo..."
                          value={formData.serieEquipo}
                          onChange={(e) => setFormData({ ...formData, serieEquipo: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        />
                      </div>
                    </div>
                  </div>

                  {/* Bento Card: Falla y Visitas en Sitio */}
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs space-y-3">
                    <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                      Falla y Bitácora de Visitas
                    </h3>
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] font-semibold text-slate-500">Falla Reportada</label>
                      <textarea
                        rows={2}
                        placeholder="Descripción detallada de la falla..."
                        value={formData.fallaReportada}
                        onChange={(e) => setFormData({ ...formData, fallaReportada: e.target.value })}
                        className="text-xs border border-slate-200 rounded-lg p-2 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white resize-none"
                      />
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Fecha Visita</label>
                        <input
                          type="date"
                          value={formData.fechaVisita}
                          onChange={(e) => setFormData({ ...formData, fechaVisita: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        />
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Hora Visita</label>
                        <select
                          value={formData.horaVisita}
                          onChange={(e) => setFormData({ ...formData, horaVisita: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white truncate"
                        >
                          {HORA_VISITA_OPTIONS.map((h) => (
                            <option key={h} value={h}>
                              {h}
                            </option>
                          ))}
                        </select>
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">Técnico</label>
                        <select
                          value={formData.tecnico}
                          onChange={(e) => setFormData({ ...formData, tecnico: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white truncate"
                        >
                          {STAFF_LIST.map((staff) => (
                            <option key={staff} value={staff}>
                              {staff}
                            </option>
                          ))}
                        </select>
                      </div>
                    </div>
                    {/* Bitácora de 3 visitas */}
                    <div className="space-y-2 pt-2 border-t border-slate-100">
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-slate-500">1ª Visita (Diagnóstico)</label>
                        <textarea
                          rows={2}
                          placeholder="Diagnóstico en sitio, mediciones y hallazgos..."
                          value={formData.detalles1erVisita}
                          onChange={(e) => setFormData({ ...formData, detalles1erVisita: e.target.value })}
                          className="text-xs border border-slate-200 rounded-lg p-2 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white resize-none"
                        />
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="flex flex-col gap-1">
                          <label className="text-[10px] font-semibold text-slate-500">2ª Visita (Opcional)</label>
                          <textarea
                            rows={1}
                            placeholder="Instalación y pruebas..."
                            value={formData.detalles2daVisita}
                            onChange={(e) => setFormData({ ...formData, detalles2daVisita: e.target.value })}
                            className="text-xs border border-slate-200 rounded-lg p-2 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white resize-none"
                          />
                        </div>
                        <div className="flex flex-col gap-1">
                          <label className="text-[10px] font-semibold text-slate-500">3ª Visita (Cierre)</label>
                          <textarea
                            rows={1}
                            placeholder="Prueba final de operación..."
                            value={formData.detalles3eraVisita}
                            onChange={(e) => setFormData({ ...formData, detalles3eraVisita: e.target.value })}
                            className="text-xs border border-slate-200 rounded-lg p-2 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white resize-none"
                          />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                {/* Column 3: Right (3 cols) */}
                <div className="md:col-span-3 space-y-4 flex flex-col">
                  {/* Bento Card Highlight: Costeo y Presupuesto */}
                  <div className="bg-indigo-600 p-5 rounded-xl text-white shadow-lg space-y-3">
                    <h3 className="text-[10px] font-bold opacity-80 uppercase tracking-wider mb-2">
                      Presupuesto en Sitio
                    </h3>
                    <div className="space-y-3 text-xs">
                      <div>
                        <label className="text-[10px] opacity-80 font-semibold block mb-1">
                          Presupuesto Total ($)
                        </label>
                        <input
                          type="number"
                          step="0.01"
                          min="0"
                          value={formData.presupuesto || ''}
                          onChange={(e) => setFormData({ ...formData, presupuesto: parseFloat(e.target.value) || 0 })}
                          className="w-full bg-white/10 border border-white/20 rounded-lg px-2.5 py-1.5 text-white font-black text-lg focus:outline-none focus:bg-white/20"
                          placeholder="0.00"
                        />
                      </div>
                      <div>
                        <label className="text-[10px] opacity-80 font-semibold block mb-1">Partes Solicitadas</label>
                        <input
                          type="text"
                          placeholder="Refacciones..."
                          value={formData.partesSolicitadas}
                          onChange={(e) => setFormData({ ...formData, partesSolicitadas: e.target.value })}
                          className="w-full bg-white/10 border border-white/20 rounded-lg px-2 py-1 text-white text-xs placeholder:text-indigo-200 focus:outline-none"
                        />
                      </div>
                      <div>
                        <label className="text-[10px] opacity-80 font-semibold block mb-1"># de Pedido</label>
                        <input
                          type="text"
                          placeholder="Ej. PED-DOM-331"
                          value={formData.numeroPedido}
                          onChange={(e) => setFormData({ ...formData, numeroPedido: e.target.value })}
                          className="w-full bg-white/10 border border-white/20 rounded-lg px-2 py-1 text-white text-xs placeholder:text-indigo-200 focus:outline-none"
                        />
                      </div>
                      <div>
                        <label className="text-[10px] opacity-80 font-semibold block mb-1">No. Orden Servicio</label>
                        <input
                          type="text"
                          placeholder="Ej. OS-DOM-44"
                          value={formData.numeroOrdenServicio}
                          onChange={(e) => setFormData({ ...formData, numeroOrdenServicio: e.target.value })}
                          className="w-full bg-white/10 border border-white/20 rounded-lg px-2 py-1 text-white text-xs placeholder:text-indigo-200 focus:outline-none"
                        />
                      </div>
                    </div>
                  </div>

                  {/* Bento Card: Seguridad Confidencial */}
                  <div className="bg-amber-50 p-4 rounded-xl border border-amber-200 shadow-xs flex-1 flex flex-col justify-between">
                    <div>
                      <div className="flex items-center gap-1.5 text-amber-900 font-bold text-[11px] mb-2">
                        <Lock className="w-3.5 h-3.5 text-amber-600" />
                        <span>Acceso & Seguridad</span>
                      </div>
                      <textarea
                        rows={3}
                        placeholder="Claves de portón, acceso a caseta, notas de seguridad..."
                        value={formData.informacionConfidencial}
                        onChange={(e) => setFormData({ ...formData, informacionConfidencial: e.target.value })}
                        className="w-full text-[11px] text-amber-900 border border-amber-200 rounded-lg p-2 bg-white focus:outline-none focus:ring-2 focus:ring-amber-500 resize-none"
                      />
                    </div>
                  </div>
                </div>
              </div>

              {/* Bento Footer Action Bar */}
              <div className="flex items-center justify-end gap-2 pt-4 mt-4 border-t border-slate-200">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-600 hover:bg-slate-50"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold shadow-xs flex items-center gap-1.5"
                >
                  <Save className="w-4 h-4" />
                  <span>Guardar Reporte en Sitio</span>
                </button>
              </div>
            </form>
          </div>
        </div>
      )}

      {/* Format Print / Preview Modal */}
      {previewRecord && (
        <FormatPrintPreview
          module="reporte_sitio"
          record={previewRecord}
          company={company}
          isOpen={Boolean(previewRecord)}
          onClose={() => setPreviewRecord(null)}
        />
      )}
    </div>
  );
};
